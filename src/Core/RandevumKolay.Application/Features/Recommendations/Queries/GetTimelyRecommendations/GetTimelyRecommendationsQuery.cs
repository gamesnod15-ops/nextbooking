...
namespace RandevumKolay.Application.Features.Recommendations.Queries.GetTimelyRecommendations;

public record GetTimelyRecommendationsQuery(
    Guid CustomerId,
    int Count = 5) : IRequest<List<RecommendationDto>>;

public sealed class GetTimelyRecommendationsQueryHandler
    : IRequestHandler<GetTimelyRecommendationsQuery, List<RecommendationDto>>
{
    private readonly IRecommendationService _recommendationService;
    private readonly ICurrentTenantService _tenantService;

    public GetTimelyRecommendationsQueryHandler(
    ...
    public async Task<List<RecommendationDto>> Handle(
        GetTimelyRecommendationsQuery request,
        CancellationToken cancellationToken)
    {
        var recommendations = await _recommendationService.GetTimelyRecommendationsAsync(
