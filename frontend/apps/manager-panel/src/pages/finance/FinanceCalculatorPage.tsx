...
const STORAGE_KEY = 'manager_finance_calc'
...
export function FinanceCalculatorPage() {
  ...
  const totalMonthlyExpenses = useMemo(() => expenses.reduce((sum, e) => sum + e.amount, 0), [expenses])
  const totalYearlyExpenses = totalMonthlyExpenses * 12
  const netMonthly = totalMonthlyRevenue - totalMonthlyExpenses
  const netYearly = netMonthly * 12
  const profitMargin = totalMonthlyRevenue > 0 ? ((netMonthly / totalMonthlyRevenue) * 100).toFixed(1) : '0.0'
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Finans Hesaplayıcı" description="Aylık ve yıllık gelir-gider analizi" />

      {/* Summary Cards */}
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <SummaryCard icon={<TrendingUp className="h-5 w-5 text-emerald-600" />} label="Aylık Gelir" value={formatCurrency(totalMonthlyRevenue)} bg="bg-emerald-50 border-emerald-200" />
        <SummaryCard icon={<TrendingDown className="h-5 w-5 text-red-500" />} label="Aylık Gider" value={formatCurrency(totalMonthlyExpenses)} bg="bg-red-50 border-red-200" />
        ...
        <SummaryCard icon={<BarChart3 className="h-5 w-5 text-blue-600" />} label="Net Yıllık Kâr" value={formatCurrency(netYearly)} bg="bg-blue-50 border-blue-200" valueClass="text-blue-700" />
      </div>
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
        ...
        <section className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
          <div className="flex items-center justify-between mb-5">
            ...
            <button onClick={addPackage} className="flex items-center gap-1 rounded-lg bg-emerald-50 px-3 py-1.5 text-xs font-semibold text-emerald-700 hover:bg-emerald-100 transition-colors">
          ...
          <div className="space-y-3">
            ...
            {packages.map((pkg) => (
              <div key={pkg.id} className="grid grid-cols-[1fr_100px_90px_36px] gap-2 items-center">
                <input type="text" value={pkg.name} onChange={(e) => updatePackageName(pkg.id, e.target.value)}
                  className="rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-sm font-medium text-gray-900 focus:border-blue-400 focus:ring-1 focus:ring-blue-400 outline-none" />
                <div className="relative">
                  <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-xs text-gray-400">₺</span>
                  <input type="number" value={pkg.price || ''} onChange={(e) => updatePackage(pkg.id, 'price', Number(e.target.value) || 0)}
                    disabled={pkg.name === 'Kurumsal'}
                    className="w-full rounded-lg border border-gray-200 bg-gray-50 pl-6 pr-2 py-2 text-sm text-right text-gray-900 focus:border-blue-400 focus:ring-1 focus:ring-blue-400 outline-none disabled:bg-gray-100 disabled:text-gray-400" />
                </div>
                <input type="number" value={pkg.customers || ''} onChange={(e) => updatePackage(pkg.id, 'customers', Number(e.target.value) || 0)}
                  placeholder="0"
                  className="w-full rounded-lg border border-gray-200 bg-gray-50 px-2 py-2 text-sm text-right text-gray-900 focus:border-blue-400 focus:ring-1 focus:ring-blue-400 outline-none" />
                <button onClick={() => removePackage(pkg.id)} className="flex h-8 w-8 items-center justify-center rounded-lg text-gray-400 hover:bg-red-50 hover:text-red-500 transition-colors">
        ...
        <section className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
          <div className="flex items-center justify-between mb-5">
            ...
            <button onClick={addExpense} className="flex items-center gap-1 rounded-lg bg-red-50 px-3 py-1.5 text-xs font-semibold text-red-700 hover:bg-red-100 transition-colors">
              <Plus className="h-3.5 w-3.5" /> Ekle
            </button>
          </div>
          <div className="space-y-3">
            ...
            {expenses.map((exp) => (
              <div key={exp.id} className="grid grid-cols-[1fr_130px_36px] gap-2 items-center">
                <input type="text" value={exp.label} onChange={(e) => updateExpenseLabel(exp.id, e.target.value)}
                  className="rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-gray-900 focus:border-blue-400 focus:ring-1 focus:ring-blue-400 outline-none" />
                <div className="relative">
                  <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-xs text-gray-400">₺</span>
                  <input type="number" value={exp.amount || ''} onChange={(e) => updateExpense(exp.id, Number(e.target.value) || 0)}
                    placeholder="0"
                    className="w-full rounded-lg border border-gray-200 bg-gray-50 pl-6 pr-2 py-2 text-sm text-right text-gray-900 focus:border-blue-400 focus:ring-1 focus:ring-blue-400 outline-none" />
                </div>
                <button onClick={() => removeExpense(exp.id)} className="flex h-8 w-8 items-center justify-center rounded-lg text-gray-400 hover:bg-red-50 hover:text-red-500 transition-colors">
                  <Trash2 className="h-3.5 w-3.5" />
                </button>
              </div>
            ))}
